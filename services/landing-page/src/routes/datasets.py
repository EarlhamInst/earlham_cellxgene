...
def list_datasets():
    ...
    try:
        ...
        if organism:
            datasets = [
                ds for ds in datasets if ds.organism.lower() == organism.lower()
            ]
        ...
        if sort_by == "cell_count":
            datasets = sorted(
                datasets, key=lambda ds: ds.cell_count or 0, reverse=(order == "desc")
            )
...
def get_dataset_metadata(dataset_id: str):
    ...
    try:
        ...
        logger.error(
            f"Error getting metadata for {dataset_id}: {str(e)}", exc_info=True
        )
        error_response, status_code = format_error_response(e, 500)
        return jsonify(error_response), status_code
...
def keepalive_dataset(dataset_id: str):
    ...
    try:
        ...
        logger.error(
            f"Error updating keep-alive for {dataset_id}: {str(e)}", exc_info=True
        )
        error_response, status_code = format_error_response(e, 500)
        return jsonify(error_response), status_code
...
def get_dataset_status(dataset_id: str):
    ...
    try:
        ...
        if container_info is None and port is None:
            logger.info(
                f"{dataset_id}: No container info and no port - returning not_running"
            )
            return (
                jsonify(
                    {
        ...
        if container_info is None and port is not None:
            logger.warning(
                f"{dataset_id}: Has port {port} but no container info - likely killed"
            )
            return (
                jsonify(
                    {
        ...
        if container_info:
            logger.info(
                f"{dataset_id}: Container status={container_info.get('status')}, oom_killed={container_info.get('oom_killed')}, exit_code={container_info.get('exit_code')}"
            )
