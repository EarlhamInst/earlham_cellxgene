...
class DatasetScanner:
    ...
    def scan(self, fail_on_invalid: bool = False) -> Tuple[List[Dataset], List[Tuple[str, List[str]]]]:
        """
    ...
    def _load_dataset(self, h5ad_path: Path) -> Dataset:
        ...
        try:
            ...
            raise FileAccessError(
                str(h5ad_path),
                'read',
                f'Failed to read h5ad file: {str(e)}'
            )
        ...
        try:
            with open(h5ad_path, 'rb') as f:
                ...
                if header != hdf5_signature:
                    return False, [f"File does not have valid HDF5/h5ad header: {h5ad_path}"]
        except Exception as e:
