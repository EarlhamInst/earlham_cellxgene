...
class CellxgeneContainerManager:
    ...
    def launch_dataset(self, dataset_id: str, dataset_filename: str) -> int:
        ...
        """
        self.logger.info(
            f"Launch request for dataset: {dataset_id}, filename: {dataset_filename}"
        )
        ...
        if dataset_id in self.active_containers:
            ...
            try:
                ...
                if container.status == "running":
                    self.logger.info(
                        f"Container for {dataset_id} already running on port {port}"
                    )
                    # Update last accessed time
                    self.active_containers[dataset_id] = (
                        container,
                        port,
                        datetime.now(),
                    )
                    ...
                    self.logger.warning(
                        f"Container for {dataset_id} not running, restarting"
                    )
        ...
        try:
            ...
            self.logger.info(
                f"Found existing container with status: {existing_container.status}"
            )
            if existing_container.status == "running":
                # Get the port from existing container
                ports = existing_container.attrs.get("NetworkSettings", {}).get(
                    "Ports", {}
                )
                ...
                if port_bindings and len(port_bindings) > 0:
                    ...
                    self.logger.info(
                        f"Reusing existing container for {dataset_id} on port {port}"
                    )
                    return port
                else:
                    self.logger.warning(
                        f"Existing container for {dataset_id} has no port bindings, removing"
                    )
            ...
            self.logger.error(
                f"Error checking for existing container: {e}", exc_info=True
            )
        ...
        self.logger.info(
            f"Launching CellXGene for {dataset_id} ({dataset_filename}) on port {port}"
        )
        ...
        try:
            ...
            self.logger.info(
                f"Successfully launched container for {dataset_id} on port {port}"
            )
            ...
            self.logger.error(
                f"Failed to launch container for {dataset_id}: {e}", exc_info=True
            )
    ...
    def _wait_for_healthy(self, container, timeout: int = 60):
        ...
        while time.time() - start_time < timeout:
            try:
                ...
                if status == "exited":
                    ...
                    self.logger.error(
                        f"Container {container.name} exited! Exit code: {exit_code}, OOMKilled: {oom_killed}"
                    )

                    if oom_killed or exit_code == 137:
                        raise RuntimeError(
                            f"Container was killed due to out of memory (OOM). Exit code: {exit_code}"
                        )
                    elif exit_code != 0:
                        raise RuntimeError(
            ...
            try:
                ...
                if state.get("OOMKilled", False) or state.get("ExitCode") == 137:
                    raise RuntimeError(
                        f"Container was killed due to out of memory (OOM)"
                    )
            except:
                pass
        ...
        self.logger.info(
            f"Container {container_name} running, waiting for application to be ready..."
        )
        ...
        while time.time() - start_time < timeout:
            try:
                ...
                if status == "exited":
                    ...
                    self.logger.error(
                        f"Container {container.name} exited during health check! Exit code: {exit_code}, OOMKilled: {oom_killed}"
                    )
                ...
                with urllib.request.urlopen(req, timeout=2) as response:
                    if response.status in [200, 301, 302]:
                        self.logger.info(
                            f"Container {container_name} is healthy and responding"
                        )
                        return
            except RuntimeError:
                raise  # Re-raise our custom errors
    ...
    def cleanup_inactive(self, max_inactive_seconds: int = 3600):
        ...
        for dataset_id, (
            ...
            try:
                ...
                if container.status != "running":
                    self.logger.info(
                        f"Container {dataset_id} is not running, marking for removal"
                    )
                ...
                self.logger.warning(
                    f"Container {dataset_id} not found, marking for removal"
                )
                to_remove.append(dataset_id)
            except Exception as e:
                self.logger.error(
                    f"Error checking container {dataset_id}: {e}", exc_info=True
                )
                to_remove.append(dataset_id)
    ...
    def get_container_info(self, dataset_id: str) -> Optional[Dict]:
        ...
        if dataset_id in self.active_containers:
            ...
            try:
                container = self.client.containers.get(container_name)
                self.logger.info(
                    f"Found container {container_name} not in active_containers (status checking orphaned container)"
                )
            except docker.errors.NotFound:
                ...
                self.logger.error(
                    f"Error finding container {container_name}: {e}", exc_info=True
                )
                return None
        ...
        try:
            ...
            if port is None:
                ...
                if port_mapping and len(port_mapping) > 0:
                    ...
                    self.logger.debug(
                        f"Extracted port {port} from container {dataset_id}"
                    )
            ...
            self.logger.debug(
                f"Container {dataset_id} info: status={info['status']}, exit_code={info['exit_code']}, oom_killed={info['oom_killed']}, port={port}"
            )
            return info
        except docker.errors.NotFound:
            return None
