...
class DatasetMetadata:
    ...
    def from_dict(cls, data: Dict[str, Any]) -> 'DatasetMetadata':
        ...
        known_fields = {
            'name', 'description', 'organism', 'tissue', 'assay',
            'cell_count', 'gene_count', 'doi', 'publication',
            'contributors', 'version', 'date_created', 'license', 'tags'
        }
    ...
    def validate(self) -> tuple[bool, List[str]]:
        ...
        if not self.organism:
            errors.append("Field 'organism' is missing (should be present, can be 'Unknown')")
        
        if not self.tissue:
            errors.append("Field 'tissue' is missing (should be present, can be 'Unknown')")
        
        if not self.assay:
            errors.append("Field 'assay' is missing (should be present, can be 'Unknown')")
        ...
        if self.doi and not (self.doi.startswith('10.') or self.doi.startswith('http')):
            errors.append(
                f"DOI should start with '10.' or be a URL, got: {self.doi}"
            )
