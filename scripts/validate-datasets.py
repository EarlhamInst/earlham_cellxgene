    ...
    try:
        with open(filepath, 'rb') as f:
            ...
            if header != hdf5_signature:
                return False, f"File does not appear to be valid HDF5/h5ad format: {filepath}"
...
def validate_metadata_schema(metadata: Dict) -> Tuple[bool, Optional[str]]:
    ...
    for field in ['organism', 'tissue', 'assay']:
        if field not in metadata:
            return False, f"Missing field: {field} (should be present, can be 'Unknown')"
...
def extract_metadata_from_h5ad(h5ad_path: Path) -> Tuple[Optional[Dict], Optional[str]]:
    ...
    try:
        ...
        metadata['name'] = metadata.get('name', metadata.get('title', h5ad_path.stem.replace('_', ' ').replace('-', ' ').title()))
        metadata['description'] = metadata.get('description', metadata.get('summary', f'Dataset with {adata.n_obs:,} cells and {adata.n_vars:,} features'))
        ...
        if 'organism' not in metadata:
            ...
            elif 'organism' in adata.obs.columns and len(adata.obs['organism'].unique()) == 1:
        ...
        if 'tissue' not in metadata:
            ...
            elif 'tissue' in adata.obs.columns and len(adata.obs['tissue'].unique()) == 1:
        ...
        optional_fields = ['doi', 'publication', 'contributors', 'version', 'date_created', 'license', 'tags']
...
Examples:
  ...
  %(prog)s --fail-fast
    ...
    try:
        ...
        if invalid_count > 0:
            if args.fail_fast:
                print("\n❌ Validation failed. Fix errors above before deploying.", file=sys.stderr)
                sys.exit(1)
            else:
                print("\n⚠️  Some datasets are invalid. They will be skipped at runtime.", file=sys.stderr)
                sys.exit(0)
        else:
            if valid_count == 0:
        ...
        print(f"\nCreate the data directory with: mkdir -p {args.data_dir}", file=sys.stderr)
        sys.exit(1)
